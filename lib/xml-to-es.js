...
exports.Parser = function (config) {
  ...
  function openZip(fpath) {
    var gzStream = fs.createReadStream(fpath, 'utf8');
    input = zlib.createGunzip();
    gzStream.pipe(unzipper);
    var rl = fs.createReadStream(unzipper);
    //closeZip = function () {
    //  rl.close();
      //unzipper.close();
      //input.close();
  ...
  self.processFiles = function (cb) {
    ...
    function doOne() {
      ...
      if (/\.gz$/.test(infile)){
        input = openZip(infile);
        input.on('readable', whenReady);
      } else {
