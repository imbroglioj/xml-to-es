...
exports.Parser = function (config) {
  ...
  function processPossibleGoodDoc(teRegex, s, eltClose, cb) {
    teRegex.lastIndex = 0;
    ...
    function doOneFile() {
      ...
      function checkStart(box) {
        ...
        if (!box.xml.length) { // start of file
          ...
          if (self.isHtml()) {
            ...
            if (m && m.length) {
              box.doctype = m[1];
              box.dtrx = new RegExp("(?:<!DOCTYPE[^>]*>)?\\s*<" + box.doctype + "[\\s>]"); // removed ^
      ...
      function getOneXmlDoc() {
        ...
        if (box.leftover.trim().length) {
          ...
          if (!dtClose) {
            var badClose=box.leftover.match(box.dtrx);
      ...
      function whenReady(cbLocal) {
        ...
        // log.info(new Date());
        processPossibleGoodDoc(box.dtrx, box.xml, box.enddtrx, function(err, result){
          if (err) {
            if (err === 'recycling') return setImmediate(oneDoc);
...
exports.collectFiles = function collectFiles(config, argvFiles) {
  ...
  if (missingFiles.length) log.log("The following files or directories were not found: %j", missingFiles);
