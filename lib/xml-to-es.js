...
exports.Parser = function (config) {
  ...
  function modifyJson(result, cb) {
    ...
    function postPP(err) {
      ...
      if (!config.json) {
        logger.error("No config.json. PreProcess method remove it?");
        return cb ? cb('no config.json') : null;
      }
      logger.trace("PostPP");
      result = config.json;
      delete config.json;
    ...
    function doOneFile() {
      ...
      function waitingForProc() {
        if (processedFile && processedFile != config.input.currentFile) {
          logger.debug("Waiting for processedFile:%s to equal %s", processedFile, config.input.currentFile);
          return true;
        }
        else {
      ...
      function getOneXmlDoc() {
        ...
        if (box.leftover.trim().length) {
          ...
          if (!dtClose) {
            ...
            // go get more data
            logger.info("no doc close: getting more: %s", config.input.currentFile);
          ...
          logger.info("processing doc: %s, leftover has:%d", config.input.currentFile, box.leftover.length);
