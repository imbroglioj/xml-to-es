...
exports.resolveOptions = function (argv, overrides) {
  ...
  function satisfies(sought, ext) {
        if (sought instanceof RegExp) return sought.test(ext);
        return new RegExp(sought).test(ext);
    }
    ...
    function drill(infile) {
        ...
        if (fs.statSync(infile).isDirectory()) {
            ...
            if (!config.input.fileExt || satisfies(config.input.fileExt, path.extname(infile)))
