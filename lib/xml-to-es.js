...
exports.Parser = function (config) {
  ...
  function modifyJson(result, cb) {
    ...
    config.json=result;
    if (preProcess) preProcess(config, postPP);
    else postPP();
    function postPP() {
  ...
  self.processFiles = function (cb, progressCallback) {
    ...
    function doOneFile() {
      ...
      function checkXmlDecl() {
        if (/^\s*<\?xml/.test(box.leftover)) {
          box.leftover = box.leftover.replace(/^\s*<\?xml[^>]*>/, '');
        }
      }
...
var Logger = require('./heap-logger.js').Logger;
