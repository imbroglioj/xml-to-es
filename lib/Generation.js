...
function createOutputStreamFromId(infile, json, config) {
  var fname;
  if (!json.id) fname= path.basename(infile,'.xml');
  else fname = json.id;
  // strip ending if nec. and add it on if not there
  fname = path.basename(fname, '.'+config.output.fileExt);
  //if (json.id && json.id.trim().length && json.id.indexOf(fname) < 0)
  //  fname = json.id + '-' + fname;
...
exports.Generation = function (config) {
  ...
  self.createAggregateOutputFun = function (config) {
    ...
    var output;
    var targetFileBase = config.output.filePrefix || path.basename(config.input.currentFile,
