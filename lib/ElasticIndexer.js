...
var request = require('superagent')
  ...
  , log = bunyan.createLogger({name:'ElasticIndexer', level:'INFO'})
  ;
...
exports.ElasticIndexer = function (config) {
  ...
  log.debug("indexTypeUrl=" + self.indexTypeUrl);
    ...
    fs.readFile(f, {encoding: 'utf8'}, function (err, txt) {
      ...
      try {
        ...
        if (Array.isArray(json)) {
          log.debug("Seeing array in file:" + f + ", length:" + f.length);
          function submitting() {
          ...
          log.debug("submitting object of type:" + typeof(json));
          self.submitObject(json, f, cb);
        }
      } catch (e) {
        log.error("Processing json from %s for submission.", f, e);
  ...
  self.submitObject = function submitObject(json, f, cb) {
    var id = json.id.trim();
    if (!id) {
      log.error("No id in json object from file: %s. Cannot store", f);
      return cb? cb('no id'): null;
    }
    log.debug("Putting:" + self.indexTypeUrl + id);
    var putErrorHandled;
    request.put(self.indexTypeUrl + id)
      .send(json)
      .on('error', function (err) {
        putErrorHandled = true;
        handleHttpError(err, util.format("putting file %s to %s", f, self.indexTypeUrl+id), cb);
      })
      .end(function (err, res) {
        if (err) return putErrorHandled ? (cb? setImmediate(cb,err):null)
          : handleHttpError(err, util.format("indexing document: %s with keys:%j",
          self.indexTypeUrl+id, Object.keys(json)), cb);
        if (!res.ok) {
          log.error("%s, %s, while posting from file: %s, to URL: %s, object: %s", res.status,
          ...
          log.info("indexed document: %s", id);
          if (cb) setImmediate(cb);
        }
      });
  };
      ...
      request.get(self.indexTypeUrl + '_count')
        ...
        .end(function (err, res) {
          if (err) return putErrorHandled ? null : handleHttpError(err,
            "getting doc count: " + self.indexTypeUrl, cb);
          if (!res.ok) {
            log.error('bad result from count: %s; %s', res.status, res.text || res.body);
  ...
  self.deleteIndex = function deleteIndex(mapurl, cb) {
    ...
    request.del(mapurl)
      ...
      .end(function (err, res) {
        ...
        if (!res.ok) {
          log.warn("Not able to delete objects under mapping type:%s; status: %s", mapurl,
            res ? res.statusCode : "NO response from ES on deletion");
        }
        cb ? setImmediate(cb) : log.warn("no callback given to ElasticIndexer#deleteIndex");
      });
  };
    ...
    if (config.index.mapping) {
      ...
      if (typeof config.index.mapping == 'string') {
        fs.readFile(config.index.mapping, function (err, txt) {
          if (err) {
            log.error("Could not read mapping file: %s. Attempting to continue.",
      ...
      function doMapping() {
        ...
        function putMap() {
          ...
          request.put(mapurl)
            ...
            .end(function (err, res) {
              if (err) return putErrorHandled ? null : handleHttpError(err, "putting map: " + mapurl,
                cb);
              if (!res.ok) {
                log.error("posting mapping to mapurl: %s, %s: %s: %s", mapurl,
          ...
          .end(function (err, res) {
            if (err) {
              if (!res || res.statusCode != '404')
                return putErrorHandled ? null : handleHttpError(err, "getting map: " + mapurl, cb);
            }
            if (res.ok) {
              if (res.body && Object.keys(res.body).length > 0) {
                if (config.index.clean) {
                  log.warn("Replacing existing mapping with new or default mapping."
                  ...
                  request.del(indexToDelete)
                    ...
                    .end(function (err, res) {
                      ...
                      if (err) return putErrorHandled ? null : handleHttpError(err,
                        ...
                        + mapurl, cb);
                      if (!res.ok) {
                        log.warn("Not able to delete objects under mapping type:%s", mtype);
                      }
                      putMap();
                    });
                }
              ...
              log.info("Creating new index <%s> with settings: %j",
      ...
      request.put(config.index.url)
        ...
        .end(function (err, res) {
          if (err) return putErrorHandled ? null : handleHttpError(err, "creating index: "
            + config.index.url, cb);
          if (!res.ok) {
            log.error("creating index: %s, %s: %s", config.index.url,
  ...
  self.putFiles = function putFiles(paths, cb) {
    var files = paths.filter(function (f) {
      if (!fs.existsSync(f)) {
        log.error("Input file|directory not found: %s.  (continuing if possible)", f);
        return false;
      }
      return true;
        ...
        if (fs.statSync(value).isDirectory()) {
          var fls=[];
          fs.readdirSync(value).forEach(function (f) {
            if (!config.index.ext || path.extname(f) == config.index.ext) {
              fls.push(path.join(value, f));
            }});
          if (fls && fls.length) return accum.concat(fls);
          else return accum;
        } else {
          accum.push(value);
          return accum;
      ...
      []);
    var sawErrors = 0;
    log.debug("Files: " + util.inspect(files));
    function doOne() {
      ...
      self.submitFile(file,
        function (err) {
          if (err) {
            log.error("Saw errors in file %s", file);
            sawErrors++;
            return setImmediate(doOne);
          }
          log.info("successfully submitted: %s", file);
          return setImmediate(doOne);
        });
    }
...
exports.resolveConfigOptions = function (config, clean, level) {
  ...
  log.debug("CONFIG: " + util.inspect(config));
