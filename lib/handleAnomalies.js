...
var paraq = /\n[ ]{4}"(\S+(\s|\n))*[ ]{4}/g
    ...
    , log = bunyan.createLogger({name:'handleAnomalies', level:'INFO'})
    ;
...
exports.splitForBadDocumentClose = function splitForBadDocumentClose(s, topElement, teRegex, strings) {
    ...
    while ((tmp = teRegex.exec(s))) {
        // missing end element
        if (tmp.index==0) continue; // html keeps its topelement
...
exports.supplyMissingHeadElement = function supplyMissingHeadElement(s, topElement, strings, cb) {
    if (s.length) {
        log.warn(util.format('Top element: %s not found in start of xml piece: %s.'),
            topElement, s.substring(0, 100));
        if (s.length > 20) {
            var loc = /<([^-\s!>]+)/.exec(s);
            if (topElement.score(loc[1]) > 0.5) {
                s = s.replace(loc[1], topElement);
                log.warn('*** correcting top element to: %s\n', s.substring(0, 100));
            } else {// okay just fudg it
                s = '<' + topElement + '>\n' + s;
                log.warn('*** adding top element as: %s\n', s.substring(0, 100));
            }
            strings.unshift(s);
        }
    }
