...
exports.Logger = function(includeDate){
    ...
    self.error =function () {
        try {
            ...
            var fmtCount = arguments[0].match(/[^\\]%/g);
            if (fmtCount+1 < arguments.length ) {
                var maybeErr = arguments[arguments.length - 1];
                if (typeof maybeErr=='object' && maybeErr['stack']) console.log(maybeErr.stack);
            }
        } catch (err) {
